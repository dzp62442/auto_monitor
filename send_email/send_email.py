...
def baseline():
    ...
    if boot_time is not None and uptime_minutes is not None:
...
def send_email(subject, body, smtp_server='smtp.126.com', smtp_port=465):
    ...
    try:
        ...
        if home_path:
            load_dotenv(os.path.join(home_path, ".smtp_env"))
        from_email = os.getenv('from_email')
        smtp_key = os.getenv('smtp_key')
        to_email = os.getenv('to_email')
        if not all((from_email, smtp_key, to_email)):
            raise ValueError("~/.smtp_env 中缺少邮件配置")
